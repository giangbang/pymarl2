...
class AttackUnitCommand(Command):
    ...
    def prepare_velocity(self, unit: Unit) -> None:
        if (
            not unit.has_within_attack_range(self.target)
...
class AttackMoveCommand(Command):
    ...
    def prepare_velocity(self, unit: Unit) -> np.ndarray:
        ...
        elif unit.combat_type == CombatType.DAMAGE and unit.priority_targets:
            closest_priority_target = self.__pick_target_damage(unit, True)
    ...
    def __pick_target_damage(self, unit: Unit, priority: bool = False) -> Unit:
        tgt = unit.priority_targets if priority else unit.potential_targets
        candidate = min(
            (
                (u, d)
                for u, d in tgt
                if u.hp > 0
                and (d < unit.minimum_scan_range + u.radius or u.target == unit)
            ),
            key=lambda p: (p[0].combat_type.priority, p[1]),
            default=None,
        )
    ...
    def __pick_target_healing(self, unit: Unit) -> Unit:
        return min(
            (
                u
                for u, d in unit.potential_targets
                if u.hp > 0
